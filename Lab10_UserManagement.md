...
1. ユーザーのインポートとチームへの編成
    ...
    1. ユーザー・タブを開き、`user`と入力します。<br>
       ...
       ![ユーザー登録](https://github.com/ICpTrial/ICPSelfLab/blob/master/images/Lab10/labuser.png)
    1. 作成された `lab team`のリンクを開き、`リソース`タブを開き「リソースの管理」をクリックします。
    1. アクセスを許可する 名前スペースと helmレポジトリにチェックを入れます。<br>
       ここでは 名前空間 `lab-space` と `local-chart` だけを設定します。
    1. 設定を確認して、「保存」をクリックします。
    
1. 各ユーザーでのログイン
    それぞれのユーザーでログインして、名前空間やカタログ、各種Kubernetesオブジェクトにアクセスしてみてください。
    User1~User4のパスワードは、変更をしていなければ `ChangeMe` です。

    | ユーザー | 役割| 備考 |
    |:----------|:------------|:------------|
    | admin | クラスター管理者 |クラスター全体の管理が可能です。すべての名前空間、すべてのカタログの内容にアクセス可能です。クラスター全体のダッシュボードにもアクセス可能です。 |
    | user1 | 管理者 |指定された名前空間の管理者権限です。特定のの名前空間（今回はlab-spaceのみ）、許可されたカタログにアクセス可能です。管理者権限のあるチームのユーザー編集や、各種オブジェクトの変更も可能です。 |
    | user2 | オペレーター |指定された名前空間で変更権限を持ちます。特定のの名前空間（今回はlab-spaceのみ）、許可されたカタログにアクセス可能です。チームのユーザー編集はできませんが、各種オブジェクトの変更は可能です。モニタリングやアラートにアクセスできます。 |
    | user3 | エディター|指定された名前空間で変更権限を持ちます。特定のの名前空間（今回はlab-spaceのみ）、許可されたカタログにアクセス可能です。チームのユーザー編集はできませんが、各種オブジェクトの変更が可能です。モニタリングやアラートにはアクセスできません。  |
    | user4 | ビューアー | 指定された名前空間の参照権限を持ちます。特定のの名前空間（今回はlab-spaceのみ）、許可されたカタログにアクセス可能です。各種オブジェクトの参照は可能ですが、設定の変更はできません。 |

    各ロールに許された権限の詳細は、以下をご確認ください。
