...
1. 各ユーザーでのログイン
    ...
    | ユーザー | 役割| 備考 |
    |:----------:|:------------:|:------------:|
    | admin | クラスター管理者 | admin 権限と同様クラスター全体の管理が可能です。すべての名前空間、すべてのカタログの内容にアクセス可能です。 |
    | User1 | 管理者 | admin 権限と同様クラスター全体の管理が可能です。すべての名前空間、すべてのカタログの内容にアクセス可能です。 |
    | User2 | エディター| 指定された名前空間の管理者権限です。特定のの名前空間（今回はlab-spaceのみ）、許可されたカタログにアクセス可能です。管理者権限のあるチームのユーザー編集や、各種オブジェクトの変更も可能です。 |
    | User3 | オペレーター |指定された名前空間で変更権限を持ちます。特定のの名前空間（今回はlab-spaceのみ）、許可されたカタログにアクセス可能です。チームのユーザー編集はできませんが、各種オブジェクトの変更は可能です。  |
    | User4 | ビューアー | 指定された名前空間の参照権限を持ちます。特定のの名前空間（今回はlab-spaceのみ）、許可されたカタログにアクセス可能です。各種オブジェクトの参照は可能ですが、変更はできません。 |
